...
void Board::DrawSegment(const Location & loc, Color c)
{
	assert(loc.x >= 0);
	assert(loc.x < width);
	assert(loc.y >= 0);
	assert(loc.y < height);
	gfx.DrawRectDim(loc.x*dimension, loc.y*dimension, 10, 10, c);
}
...
bool Board::isInsideTheBoard(const Location & loc) const
{
